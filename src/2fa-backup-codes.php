...
<head>
    <title><?= t('backup_codes_title') ?></title>
    <meta charset="UTF-8">
...
<body class="no-ads">
    ...
    <div class="content-wrapper">
        <div class="main max-w-2xl mx-auto">
            <div class="card">
                <h1><?= t('backup_codes_heading') ?></h1>
                <p class="entry-meta"><?= t('backup_codes_description') ?></p>
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px; margin: 20px 0;">
                    <div style="padding: 16px; border-radius: 12px; background: rgba(34, 197, 94, 0.12); border: 1px solid rgba(34, 197, 94, 0.35);">
                        ...
                        <p style="margin: 4px 0 0 0; color: #86efac; font-size: 13px;"><?= t('backup_codes_unused_label') ?></p>
                    </div>
                    <div style="padding: 16px; border-radius: 12px; background: rgba(107, 114, 128, 0.12); border: 1px solid rgba(107, 114, 128, 0.35);">
                        ...
                        <p style="margin: 4px 0 0 0; color: #9ca3af; font-size: 13px;"><?= t('backup_codes_used_label') ?></p>
                    </div>
                </div>

                <h3 style="margin-top: 24px;"><?= t('backup_codes_status_heading') ?></h3>
                <div style="margin-top: 12px;">
                    <?php if (!empty($codes)): ?>
                        <table style="width: 100%; border-collapse: collapse; font-size: 13px;">
                            <thead>
                                <tr style="border-bottom: 1px solid rgba(255,255,255,0.1);">
                                    <th style="padding: 8px; text-align: left; color: #cbd5e1;"><?= t('backup_codes_table_id') ?></th>
                                    <th style="padding: 8px; text-align: left; color: #cbd5e1;"><?= t('backup_codes_table_status') ?></th>
                                    <th style="padding: 8px; text-align: left; color: #cbd5e1;"><?= t('backup_codes_table_created') ?></th>
                                    <th style="padding: 8px; text-align: left; color: #cbd5e1;"><?= t('backup_codes_table_used') ?></th>
                                </tr>
                            </thead>
                            <tbody>
                                <?php foreach ($codes as $code): ?>
                                    <tr style="border-bottom: 1px solid rgba(255,255,255,0.05);">
                                        ...
                                        <td style="padding: 8px;">
                                            <?php if ($code['used']): ?>
                                                <span style="display: inline-block; padding: 2px 8px; background: rgba(245,158,11,0.2); border: 1px solid rgba(245,158,11,0.5); border-radius: 4px; color: #fde68a; font-size: 11px;"><?= t('backup_codes_status_used') ?></span>
                                            <?php else: ?>
                                                <span style="display: inline-block; padding: 2px 8px; background: rgba(34,197,94,0.2); border: 1px solid rgba(34,197,94,0.5); border-radius: 4px; color: #86efac; font-size: 11px;"><?= t('backup_codes_status_active') ?></span>
                                            <?php endif; ?>
                                        </td>
                    ...
                    <?php else: ?>
                        <p class="entry-meta"><?= t('backup_codes_empty') ?></p>
                    <?php endif; ?>
                </div>
