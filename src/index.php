<?php
    ...
    if ($isLoggedIn) {
        ...
        $res = $conn->query("SELECT id, firstname FROM users WHERE id = $uid LIMIT 1");
    ...
    $nameday = ($nd && $nd->num_rows > 0) ? $nd->fetch_assoc()['nevek'] : "Nincs névnap ma.";
    ...
    if (isset($_POST['rate-btn'])) {
        ...
        if ($rating >= 1 && $rating <= 5) {
            $conn->query("
                ...
                VALUES ($file_id, $currentUserId, $rating)
                ...
                <div class="content-grid grid-large">
                    ...
                    <?php while ($file = $latest_result->fetch_assoc()):
                        ...
                        <article class="card note-card" id="file-<?= $file_id ?>">
                            ...
                            <?php elseif ($ext=="docx"): ?>
                                <p><b>.docx fájl — töltsd le a megnyitáshoz.</b></p>
                            <?php endif; ?>

                            <p>Feltöltötte: <a class="uploader-name" href="profile.php?userid=<?= $file['uploaded_by'] ?>">@<?= $username ?></a></p>
                            <p><b>Átlag értékelés:</b> <?= $avg ?> (<?= $cnt ?> ért.)</p>
